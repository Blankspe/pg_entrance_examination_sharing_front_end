...
<!--						<div @click="mohusearch" class="search">搜索</div>-->
	...
	<!-- :style="{backgroundImage:this.$store.state.themeObj.top_image?'url('+this.$store.state.themeObj.top_image+')':'url(static/img/考研治愈.jpg)'}" -->
	<div class="headImgBox" >
		<div class="scene">
			<!-- <div><span id="luke"></span></div> -->
		</div>
		<div class="h-information">
<!-- this.$store.state.themeObj.head_portrait?this.$store.state.themeObj.head_portrait:'static/img/tou.png' -->


			<h2 class="h-description">

                        {{this.$store.state.themeObj.autograph?this.$store.state.themeObj.autograph:"考研经验分享平台"}}
...
export default {
    ...
    sendMessage:function(){
      this.$router.push(
		...
		getCategoryNameByIdInheader(categoryId){
		...
		routeChange: function() {
			...
			if (localStorage.getItem('userInfo') != null) { //存储用户信息
				that.haslogin = true;
			...
			this.getCategoryNameByIdInheader(categoryId)
		...
		mohusearch: function(){
			...
			this.$router.push('Search?inputbox='+this.inputbox)
		},
		turnToSearch(categoryId){

			this.categoryId = categoryId;
			this.$route.query.inputbox = null;
			this.$router.push('Search?categoryId='+categoryId);
